/*
RMSNorm CUDA kernel (previously LayerNorm), and also Residual, because sometimes they are fused

RMSNorm is a simpler normalization that only uses RMS (root mean square) without mean centering.
...
E.g., the rmsnorms are connected to the residuals so we += in rmsnorm backward.
*/
...
__global__ void layernorm_forward_kernel3(floatX* __restrict__ out, float* __restrict__ mean, float* __restrict__ rstd,
    ...
    // RMSNorm: compute RMS (root mean square) instead of mean and variance
    ...
    float rms = sqrtf(sum_sq / C + 1e-5f);
    float s = 1.0f / rms;  // rstd = 1 / rms

    // Store dummy mean (0.0) for compatibility, and rstd
    ...
    // final normalization and scaling by weight only (RMSNorm doesn't use bias)
    floatX* o = out + idx * C;
    for (int c = lane_id; c < C; c += WARP_SIZE) {
        // load and store using the .cs "streaming" hint to the compiler,
        // indicating that this data will not be reused soon, and can be streamed through the caches
        // this allows the threads to get more cache-hits for the (shared) weight parameter
        float n = s * (float)__ldcs(x+c);  // normalize by RMS (no mean subtraction)
        __stcs(o+c, (floatX)(n * (float)weight[c]));  // scale by weight (no bias)
    }
}
...
__global__ void layernorm_forward_kernel6(floatX* __restrict__ out, float* __restrict__ mean, float* __restrict__ rstd,
    ...
    // load weights into shared memory (no bias for RMSNorm)
    ...
    for(int c = threadIdx.x * x128::size; c < C; c += WARP_SIZE * x128::size) {
        ...
        x128 out_data;
        for(int k = 0; k < x128::size; ++k) {
            float n = s * (float)in_data[k]; // normalized output (no mean subtraction)
            float o = n * (float)w[k]; // scale by weight (no bias)
            out_data[k] = (floatX)o;
        }

        store128cs(out + c, out_data);
    }
    // cache dummy mean (0.0) and rstd for the backward pass later
...
__global__ void fused_residual_forward_kernel5(floatX* residual, floatX* normed, float* mean, float* rstd,
    ...
    for(int c = threadIdx.x * x128::size; c < C; c += WARP_SIZE * x128::size) {
        ...
        x128 out;
        for(int k = 0; k < x128::size; ++k) {
            float n = s * (float)res[k]; // normalized output (no mean subtraction)
            float o = n * (float)w[k]; // scale by weight (no bias)
            out[k] = o;
        }
...
__global__ void __launch_bounds__(512, 2) // todo - any warnings on Turing with only 1024 threads?
    ...
    // shared memory only for weight gradients (no bias for RMSNorm)
    ...
    for (int bt = baseIdx; bt < B * T; bt += warpsInGrid) {
        ...
        // RMSNorm backward: compute gradient terms
        // For RMSNorm: y = x / rms * w, where rms = sqrt(mean(x^2) + eps)
        // We need: dnorm_rms_mean = sum(dy * y) / C
        float dnorm_rms_mean = 0.0f;
        for (int i = warpThreadIdx * x128::size; i < C; i += WARP_SIZE * x128::size) {
            ...
            for (int k = 0; k < x128::size; k++) {
                ...
                dnorm_rms_mean += dnorm_i * (float)inp128_i[k];
            }
        }

        const float rstd_bt = rstd[bt];
        dnorm_rms_mean = warpReduceSum(dnorm_rms_mean) / C * rstd_bt;
        ...
        for (int c = 0; c < iterations_C; c++) {
            ...
            for(int o = 0; o < x128::size / f128::size; ++o) {
                ...
                for(int i = 0; i < f128::size; ++i) {
                    ...
                    float norm_bti = (float)inp128[x] * rstd_bt;  // RMSNorm: no mean subtraction
                    dweight_f[i] = norm_bti * dout_i;

                    // RMSNorm gradient: dx = (dy * w / rms) - (y * sum(dy * y) / C)
                    float dval = 0.0f;
                    dval += (float)weight128[x] * (float)dout128[x]; // term 1
                    dval -= norm_bti * dnorm_rms_mean; // term 2
    ...
    if (*tmp_flag == gridDim.x-1) {
        ...
        for (int c = warpId; c < iterations_C; c += warpsInBlock) {
            ...
            // RMSNorm: no bias gradient to write
...
void layernorm_forward(floatX* out, float* mean, float* rstd,
    ...
    // RMSNorm: only weight (no bias), so less shared memory
...
void layernorm_backward(floatX* dinp, floatX* dweight, floatX* dbias, float* scratch,
    ...
    size_t rounded_C = CEIL_DIV(C, (32 * x128::size)) * (32 * x128::size);
    // RMSNorm: only weight gradients (no bias), so less shared memory
    size_t shared_mem_size = (rounded_C + (block_size - 32) * f128::size) * sizeof(float);
