...
void attention_forward(floatX* out, floatX* qkvr, floatX* att,
                       floatX* inp,
                       int B, int T, int C, int NH, int NKV, cudaStream_t stream) {
    NVTX_RANGE_FN();
    // Note: `inp` is not needed for backward pass, so we re-use it as a scratch buffer.
    // Its contents will be overwritten by this function.
    const int block_size = 256;

    // For now, GQA with NKV == NH (Multi-Head Attention)
    // TODO: Implement true GQA when NKV < NH
    // inp is (B, T, 3C) QKV when NKV == NH
...
void attention_backward(floatX* dinp, floatX* dqkvr, floatX* datt, floatX* scratch,
    ...
    // Ensure grid dimensions are at least 1 (avoid zero dimensions)
    int grid_x = max(1, (T + 3) / 4);  // CEIL_DIV(T, 4), at least 1
    int grid_y = max(1, B * NH);         // at least 1
    softmax_autoregressive_backward_inplace_kernel<<<dim3(grid_x, grid_y), 256>>>(datt, att, B, T, C, scale);
