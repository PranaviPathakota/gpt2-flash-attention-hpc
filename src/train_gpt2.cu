...
typedef struct {
    ...
    int num_kv_heads; // number of key/value heads for GQA, e.g. 4 (if 0, defaults to num_heads for MHA)
...
void fill_in_parameter_sizes(size_t* param_sizes, size_t* param_sizeof, GPT2Config config) {
    ...
    size_t NKV = config.num_kv_heads;
    size_t HS = C / NH;  // head size

    param_sizes[0] = Vp * C; // wte
    param_sizes[1] = maxT * C; // wpe
    param_sizes[2] = L * C; // ln1w (RMSNorm weight)
    param_sizes[3] = 0; // ln1b (RMSNorm: no bias)
    // GQA: Q projection is C -> C, but K,V projections are C -> NKV*HS each
    param_sizes[4] = L * (C + 2 * NKV * HS) * C; // qkvw (Q: C*C, K: NKV*HS*C, V: NKV*HS*C)
    param_sizes[5] = L * (C + 2 * NKV * HS); // qkvb
    param_sizes[6] = L * C * C; // attprojw
    param_sizes[7] = L * C; // attprojb
    param_sizes[8] = L * C; // ln2w (RMSNorm weight)
    param_sizes[9] = 0; // ln2b (RMSNorm: no bias)
    param_sizes[10] = L * (4 * C) * C; // fcw
    param_sizes[11] = L * (4 * C); // fcb
    param_sizes[12] = L * C * (4 * C); // fcprojw
    param_sizes[13] = L * C; // fcprojb
    param_sizes[14] = C; // lnfw (RMSNorm weight)
    param_sizes[15] = 0; // lnfb (RMSNorm: no bias)
...
void fill_in_activation_sizes(const ActivationTensors* data, TensorSpec (&tensors)[NUM_ACTIVATION_TENSORS], size_t B, size_t T, GPT2Config config, int recompute) {
    ...
    // GQA: qkvr now has Q (NH*HS=C) + K (NKV*HS) + V (NKV*HS) = C + 2*NKV*HS
    tensors[17] = TENSOR_SPEC(data->qkvr, L * B * T * (C + 2 * NKV * HS));
    tensors[18] = TENSOR_SPEC(data->output, B * T * max((C + 2 * NKV * HS), max(NH*T, Vp)));
...
void gpt2_write_to_checkpoint(GPT2 *model, const char* checkpoint_path) {
    ...
    model_header[8] = model->config.num_kv_heads; // GQA: number of KV heads
...
void gpt2_build_from_checkpoint(GPT2 *model, const char* checkpoint_path, bool weight_init=true) {
    ...
    // GQA: read num_kv_heads, default to num_heads if not present (backward compatibility)
    model->config.num_kv_heads = (model_header[8] > 0) ? model_header[8] : model_header[5];
...
void gpt_build_from_descriptor(GPT2 *model, const char* descriptor) {
    ...
    // GQA: Initialize num_kv_heads to num_heads for standard MHA (can be changed later for GQA)
    if (model->config.num_kv_heads == 0) {
...
void gpt2_forward(GPT2 *model, const int* inputs, size_t B, size_t T) {
    ...
    const size_t NKV = model->config.num_kv_heads;
    const size_t C = model->config.channels;
    const size_t HS = C / NH;  // head size
    ...
    for (int l = 0; l < L; l++) {
        ...
        // GQA: QKV weights are now Q (C*C) + K (NKV*HS*C) + V (NKV*HS*C)
        size_t qkv_out_size = C + 2 * NKV * HS;
        floatX* l_qkvw = params.qkvw + l * qkv_out_size * C;
        floatX* l_qkvb = params.qkvb + l * qkv_out_size;
        ...
        floatX* l_qkvr = acts.qkvr + l * B * T * qkv_out_size;
        ...
        // GQA: matmul output size changed from 3*C to qkv_out_size
        matmul_forward_cublaslt(l_qkvr, l_ln1, l_qkvw, l_qkvb, B, T, C, qkv_out_size, main_stream);
        // TODO: cuDNN attention needs GQA support
        ...
        // GQA: matmul output size changed from 3*C to qkv_out_size
        matmul_forward_cublaslt(scratch, l_ln1, l_qkvw, l_qkvb, B, T, C, qkv_out_size, main_stream);
        attention_forward(l_atty, l_qkvr, l_att, scratch, B, T, C, NH, NKV, main_stream);
        #endif
...
void gpt2_backward_and_reduce(GPT2 *model, int* inputs, const int* targets, int grad_accum_steps, int micro_step) {
    ...
    for (int l = L-1; l >= 0; l--) {
        ...
        // get the pointers of the weights for this layer
        size_t qkv_out_size = C + 2 * NKV * HS;  // For NKV==NH, this is 3*C
        ...
        floatX* dl_qkvw = grads.qkvw + l * qkv_out_size * C;
        floatX* dl_qkvb = grads.qkvb + l * qkv_out_size;
        ...
        attention_backward(dl_bt4c, buffer_b, scratchX, buffer_a, dl_btc, l_qkvr, l_att, B, T, C, NH, NKV, main_stream);
        #endif
        if(model->recompute >= 2) {
            layernorm_forward(l_ln1, l_ln1_mean, l_ln1_rstd, residual, l_ln1w, l_ln1b, B, T, C, main_stream);
        }
        // QKV parameter gradients
        matmul_backward(dl_btc, dl_qkvw, dl_qkvb, dl_bt4c, l_ln1, l_qkvw, scratchF, B, T, C, qkv_out_size, main_stream);
...
float gpt2_calculate_grad_norm(GPT2 *model, MultiGpuConfig* multi_gpu_config) {
    ...
    if (multi_gpu_config->zero_stage == 1) {
        ...
        for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
            ...
            // Skip zero-sized tensors (e.g., RMSNorm biases that are removed)
            if (tensor.size == 0) {
                continue;
            }
            ...
            // Skip if shard size is zero (can happen with sharding or zero-sized tensors)
            if (shard.size == 0) {
                continue;
            }
...
void gpt2_update(GPT2 *model, float learning_rate, float beta1, float beta2, float eps, float weight_decay, float grad_scale, int t,
    ...
    for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
        ...
        if(init_state && model->master_weights != nullptr && shard.size > 0) {
    ...
    for (; step <= train_num_batches; step++) {
        ...
        if (multi_gpu_config.process_rank == 0 && sample_every > 0 &&
           (step > 0 && (step % sample_every) == 0 || last_step)) {
            ...
            if (eot_token < 0 || eot_token >= model.config.vocab_size) {
                printf("Warning: Invalid eot_token=%d, using 0 instead\n", eot_token);
                eot_token = 0;  // fallback to a safe token
            }
